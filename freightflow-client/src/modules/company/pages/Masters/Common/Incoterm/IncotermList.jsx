...
const IncotermList = ({ onEdit, searchQuery = '', viewMode = 'table', refreshTrigger = 0 , onTotalCountChange, statusFilter = 'ALL STATUS'}) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await commonService.deleteIncoterm(itemToDelete.id);
      setIsDeleteModalOpen(false);
      setItemToDelete(null);
      setIncoterms(prev => prev.filter(item => item.id !== itemToDelete.id));
    } catch (error) {
  ...
  const fetchIncoterms = async () => {
    setIsLoading(true);
    try {
      const data = await commonService.getIncoterms({ page: 1, limit: 10000 });
      let termData = [];
  ...
  const filteredList = incoterms.filter(item => {
  ...
  if (viewMode === 'card') {
    ...
    <>
      ...
      <ConfirmDeleteModal
        ...
        itemName={itemToDelete ? itemToDelete.incoterm_name || itemToDelete.name || itemToDelete.code : ''}
