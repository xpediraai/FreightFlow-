...
const CustomerList = ({ onEdit, searchQuery = '', viewMode = 'table', refreshTrigger = 0, onTotalCountChange, statusFilter = 'ALL STATUS' }) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await businessService.deleteCustomer(itemToDelete.id);
      setIsDeleteModalOpen(false);
      setItemToDelete(null);
      setCustomers(prev => prev.filter(item => item.id !== itemToDelete.id));
    } catch (error) {
  ...
  if (viewMode === 'card') {
    ...
    <>
      ...
      <ConfirmDeleteModal
        ...
        itemName={itemToDelete ? itemToDelete.customer_name || itemToDelete.name || itemToDelete.code : ''}
