...
const VehicleList = ({ onEdit, searchQuery = '', viewMode = 'table', refreshTrigger = 0 , onTotalCountChange, statusFilter = 'ALL STATUS'}) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await logisticsService.deleteVehicle(itemToDelete.id);
      setIsDeleteModalOpen(false);
      setItemToDelete(null);
      setVehicles(prev => prev.filter(item => item.id !== itemToDelete.id));
    } catch (error) {
  ...
  if (viewMode === 'card') {
    ...
    <>
      ...
      <ConfirmDeleteModal
        ...
        itemName={itemToDelete ? itemToDelete.vehicle_number || itemToDelete.name || itemToDelete.code : ''}
