...
const PortList = ({ onEdit, searchQuery = '', viewMode = 'table', refreshTrigger = 0 , onTotalCountChange, statusFilter = 'ALL STATUS'}) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await logisticsService.deletePort(itemToDelete.id);
      setIsDeleteModalOpen(false);
      setItemToDelete(null);
      setPorts(prev => prev.filter(item => item.id !== itemToDelete.id));
    } catch (error) {
  ...
  if (viewMode === 'card') {
    ...
    <>
      ...
      <ConfirmDeleteModal
        ...
        itemName={itemToDelete ? itemToDelete.port_name || itemToDelete.name || itemToDelete.code : ''}
