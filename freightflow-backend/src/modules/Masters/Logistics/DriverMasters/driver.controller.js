 ...
 * @description HTTP layer for Driver APIs.
 */
const { 
    createDriverSchema, 
    updateDriverSchema, 
...
const create = async (req, res) => {
    try {
        ...
        const newRecord = await driverService.createDriver(companyId, value, userId, reqInfo);

        return res.status(201).json(successResponse("DRIVER_CREATED", "Driver created successfully.", "Created.", newRecord));
    } catch (err) {
...
const list = async (req, res) => {
    try {
        ...
        const data = await driverService.getDrivers(req.user.company_id, value);
        return res.status(200).json(successResponse("DRIVERS_FETCHED", "Drivers fetched successfully.", "Retrieved.", data));
    } catch (err) {
...
const getById = async (req, res) => {
    try {
        const record = await driverService.getDriverById(req.user.company_id, req.params.id);
        return res.status(200).json(successResponse("DRIVER_FETCHED", "Driver fetched successfully.", "Retrieved.", record));
    } catch (err) {
        if (err.message === "Driver not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
        return res.status(500).json(errorResponse("INTERNAL_SERVER_ERROR", err.message, "Failed to fetch."));
    }
};
...
const update = async (req, res) => {
    try {
        ...
        const reqInfo = { ip: req.ip, userAgent: req.headers["user-agent"] };
        const record = await driverService.updateDriver(req.user.company_id, req.params.id, value, req.user.user_id, reqInfo);

        return res.status(200).json(successResponse("DRIVER_UPDATED", "Driver updated successfully.", "Updated.", record));
    } catch (err) {
        if (err.message === "Driver not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
...
const changeStatus = async (req, res) => {
    try {
        ...
        const record = await driverService.changeStatus(req.user.company_id, req.params.id, value.status, req.user.user_id);
...
const remove = async (req, res) => {
    try {
        await driverService.deleteDriver(req.user.company_id, req.params.id);
        return res.status(200).json(successResponse("DRIVER_DELETED", "Driver deleted successfully.", "Deleted.", null));
    } catch (err) {
        if (err.message === "Driver not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
