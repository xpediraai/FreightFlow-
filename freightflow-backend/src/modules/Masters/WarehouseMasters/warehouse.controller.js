 ...
 * @description HTTP layer for Warehouse APIs.
 */
const { 
...
const create = async (req, res) => {
    try {
        ...
        const newRecord = await warehouseService.createWarehouse(companyId, value, userId, reqInfo);

        return res.status(201).json(successResponse("WAREHOUSE_CREATED", "Warehouse created successfully.", "Created.", newRecord));
    } catch (err) {
...
const list = async (req, res) => {
    try {
        ...
        const data = await warehouseService.getWarehouses(req.user.company_id, value);
        return res.status(200).json(successResponse("WAREHOUSES_FETCHED", "Warehouses fetched successfully.", "Retrieved.", data));
    } catch (err) {
...
const getById = async (req, res) => {
    try {
        const record = await warehouseService.getWarehouseById(req.user.company_id, req.params.id);
        return res.status(200).json(successResponse("WAREHOUSE_FETCHED", "Warehouse fetched successfully.", "Retrieved.", record));
    } catch (err) {
        if (err.message === "Warehouse not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
        return res.status(500).json(errorResponse("INTERNAL_SERVER_ERROR", err.message, "Failed to fetch."));
    }
};
...
const update = async (req, res) => {
    try {
        ...
        const reqInfo = { ip: req.ip, userAgent: req.headers["user-agent"] };
        const record = await warehouseService.updateWarehouse(req.user.company_id, req.params.id, value, req.user.user_id, reqInfo);

        return res.status(200).json(successResponse("WAREHOUSE_UPDATED", "Warehouse updated successfully.", "Updated.", record));
    } catch (err) {
        if (err.message === "Warehouse not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
...
const changeStatus = async (req, res) => {
    try {
        ...
        const record = await warehouseService.changeStatus(req.user.company_id, req.params.id, value.status, req.user.user_id, {});
...
const remove = async (req, res) => {
    try {
        await warehouseService.deleteWarehouse(req.user.company_id, req.params.id, req.user.user_id, {});
        return res.status(200).json(successResponse("WAREHOUSE_DELETED", "Warehouse deleted successfully.", "Deleted.", null));
    } catch (err) {
        if (err.message === "Warehouse not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
