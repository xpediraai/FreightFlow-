 ...
 * @description HTTP layer for Vendor APIs.
 */
const { 
    createVendorSchema, 
    updateVendorSchema, 
...
const create = async (req, res) => {
    try {
        ...
        const newRecord = await vendorService.createVendor(companyId, value, userId, reqInfo);

        return res.status(201).json(successResponse("VENDOR_CREATED", "Vendor created successfully.", "Created.", newRecord));
    } catch (err) {
...
const list = async (req, res) => {
    try {
        ...
        const data = await vendorService.getVendors(req.user.company_id, value);
        return res.status(200).json(successResponse("VENDORS_FETCHED", "Vendors fetched successfully.", "Retrieved.", data));
    } catch (err) {
...
const getById = async (req, res) => {
    try {
        const record = await vendorService.getVendorById(req.user.company_id, req.params.id);
        return res.status(200).json(successResponse("VENDOR_FETCHED", "Vendor fetched successfully.", "Retrieved.", record));
    } catch (err) {
        if (err.message === "Vendor not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
        return res.status(500).json(errorResponse("INTERNAL_SERVER_ERROR", err.message, "Failed to fetch."));
    }
};
...
const update = async (req, res) => {
    try {
        ...
        const reqInfo = { ip: req.ip, userAgent: req.headers["user-agent"] };
        const record = await vendorService.updateVendor(req.user.company_id, req.params.id, value, req.user.user_id, reqInfo);

        return res.status(200).json(successResponse("VENDOR_UPDATED", "Vendor updated successfully.", "Updated.", record));
    } catch (err) {
        if (err.message === "Vendor not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
...
const changeStatus = async (req, res) => {
    try {
        ...
        const record = await vendorService.changeStatus(req.user.company_id, req.params.id, value.status, req.user.user_id, {});
...
const remove = async (req, res) => {
    try {
        await vendorService.deleteVendor(req.user.company_id, req.params.id, req.user.user_id, {});
        return res.status(200).json(successResponse("VENDOR_DELETED", "Vendor deleted successfully.", "Deleted.", null));
    } catch (err) {
        if (err.message === "Vendor not found.") return res.status(404).json(errorResponse("NOT_FOUND", err.message, err.message));
